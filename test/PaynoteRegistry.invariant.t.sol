...
contract PaynoteRegistryHandler is Test {
    ...
    function attachNote(
        ...
        if (targetTxHash == bytes32(0)) targetTxHash = keccak256(abi.encodePacked(senderSeed));
        if (referenceHash == bytes32(0)) referenceHash = keccak256(abi.encodePacked(targetTxHash));
    ...
    function getAttachedNotes() external view returns (address[] memory, bytes32[] memory) {
